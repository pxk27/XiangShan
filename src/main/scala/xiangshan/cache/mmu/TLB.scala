  ...
  with HasPerfEvents
{
  ...
  def perm_check(perm: TlbPermBundle, cmd: UInt, spm: TlbPMBundle, spm_v: Bool, idx: Int, nDups: Int, hlvx: Bool, mode: UInt, mxr: Bool, sum: Bool, s2xlate: Bool) = {
    ...
    val ldPf = (Mux(onlyS2xlate && s2xlate, false.B, ldPermFail) || pf) && (TlbCmd.isRead(cmd) && !TlbCmd.isAmo(cmd))
    val stPf = (Mux(onlyS2xlate && s2xlate, false.B, stPermFail) || pf) && (TlbCmd.isWrite(cmd) || TlbCmd.isAmo(cmd))
    val instrPf = (Mux(onlyS2xlate && s2xlate, false.B, instrPermFail) || pf) && TlbCmd.isExec(cmd)
