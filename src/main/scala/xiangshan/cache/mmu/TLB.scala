  ...
  with HasPerfEvents
{
  ...
  val need_gpa = readResult.map(_._9)
  ...
  def TLBRead(i: Int) = {
    ...
    val need_gpa = Mux(p_hit, false.B, !(s2xlate === onlyStage2) && (resp(i).bits.excp(0).gpf.ld || resp(i).bits.excp(0).gpf.st || resp(i).bits.excp(0).gpf.instr))
    val e_hit = e_hit_tmp && !need_gpa
    val e_super_hit = e_super_hit_tmp && !need_gpa
    val hit = e_hit || p_hit
    val miss = !hit && enable
    ...
    (hit, miss, pmp_paddr, static_pm, static_pm_valid, perm, g_perm, s2xlate, need_gpa)
  }
  ...
  def handle_block(idx: Int): Unit = {
    ...
    when (io.ptw.resp.fire() && hit && req_out_v(idx) && portTranslateEnable(idx)) {
      ...
      for (d <- 0 until nRespDups) {
        ...
        perm_check(stage1.entry.perm.get(), req_out(idx).cmd, 0.U.asTypeOf(new TlbPMBundle), false.B, idx, d, stage2.entry.perm, req_out(idx).hlvx, s2xlate)
      }
